...
и создание браузера для каждого теста во втором тест-сьюте. Сохраните следующий код в файл test_fixture1.py  и
запустите его с помощью PyTest. Не забудьте указать параметр -s, чтобы увидеть текст, который выводится командой print().
...
# Объявляю первый класс - тест-сьюит
class TestMainPage1():
    @classmethod
...
class TestMainPage2():
    # Когда определен метод setUp(), бегун теста будет выполнять этот метод перед каждым испытанием.
    def setup_method(self):
        print("start browser for test..")
        self.browser = webdriver.Chrome()
    # Если определен метод tearDown(), то Test runner будет вызывать этот метод после каждого теста.
